...
print_ld(split_ldsc, DIR + '/simulate_test/ldscore/twold_firstfile', [M_two[0]])
split_ldsc = ld.copy()
split_ldsc['LD'] = two_ldsc[:, 1]  # both have same colname to test that this is ok
print_ld(split_ldsc, DIR + '/simulate_test/ldscore/twold_secondfile', [M_two[1]])

# 1 LD Score 1 file
ldsc = ld.copy()
ldsc['LD'] = single_ldsc
print_ld(ldsc, DIR + '/simulate_test/ldscore/oneld_onefile', [M])

# 2 LD Scores 1 file
ldsc = ld.copy()
ldsc['LD1'] = two_ldsc[:, 0]
ldsc['LD2'] = two_ldsc[:, 1]
print_ld(ldsc, DIR + '/simulate_test/ldscore/twold_onefile', M_two)

# Weight LD Scores
w_ld = ld.copy()
w_ld['LD'] = np.ones(N_SNP)
w_ld.to_csv(DIR + '/simulate_test/ldscore/w.l2.ldscore',
...
for i in range(N_SIMS):
    ...
    dfi.to_csv(DIR + '/simulate_test/sumstats/' + str(i),
